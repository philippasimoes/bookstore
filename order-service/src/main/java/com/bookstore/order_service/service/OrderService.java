...
public class OrderService {
    ...
    /**
     * Edit the order status - possible options: OPEN, READY_TO_PAY and CANCELLED (status
     * PAID, SHIPPED and DELIVERED will be added in {@link #setOrderStatusToPaid(String) setOrderStatusToPaid},
     * {@link #setOrderStatusToShipped(String) setOrderStatusToShipped} and
     * {@link #setOrderStatusToDelivered(String) setOrderStatusToDelivered}).
     *
    ...
    public OrderDto editOrderStatus(int id, OrderStatus orderStatus) {
        ...
        if (order.isPresent()) {
            ...
            switch (orderStatus) {
                ...
                case CANCELLED -> setOrderStatusToCancelled(orderStatus, order.get());
    ...
    public void consumeShippedEvents(String message) {
        ...
        try {
            ...
            LOGGER.log(Level.ERROR, ERROR_PROCESSING_MESSAGE, e);
            e.getMessage();
        }
    }

    @RabbitListener(queues = "${rabbitmq.queue.event.delivered.name}")
    public void consumeDeliveredEvents(String message) {

        LOGGER.log(Level.INFO, "Received message in delivered events queue: {}", message);

        try {
            setOrderStatusToDelivered(message);
        } catch (JsonProcessingException e) {
            LOGGER.log(Level.ERROR, ERROR_PROCESSING_MESSAGE, e);
            e.getMessage();
        }
    ...
    private void setOrderStatusToShipped(String message) throws JsonProcessingException {
        ...
        Optional<Order> order = orderRepository.findById(Integer.parseInt(map.get(ORDER_ID)));
    ...
    /**
     * Change {@link OrderStatus} to Cancelled - the order cannot be edited anymore.
     *
     * @param orderStatus the desired status (Cancelled).
     * @param order       the order to edit.
     */
    private void setOrderStatusToCancelled(OrderStatus orderStatus, Order order) {
    ...
    /**
     * Order was delivered to customer and the database is updated with new information about the
     * order.
     *
     * @param message the message containing the order identifier and other relevant data.
     */
    private void setOrderStatusToDelivered(String message) throws JsonProcessingException {
        Map<String, String> map;
        ...
        if (order.isPresent()) {
            ...
            order.get().setEditable(false);
            order.get().setDeliveredDate(Timestamp.valueOf(map.get("deliveredDate")));
            orderRepository.save(order.get());
    ...
    private void setOrderStatusToPaid(String message) throws JsonProcessingException {
        ...
        Optional<Order> order = orderRepository.findById(map.get(ORDER_ID));
